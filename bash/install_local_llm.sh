...
TELEMETRY_DIR="${GENIE_TELEMETRY_DIR:-${LOCAL_LLM_DIR}/telemetry}"
...
read_repo_env_value() {
  ...
  local value
  for key in "$@"; do
    for env_file in "$ACCESS_ENV_FILE" "$CONF_ENV_FILE" "$LEGACY_DOCKER_ENV_FILE_PATH" "$LEGACY_ROOT_ENV_FILE_PATH"; do
...
REPO_NVIDIA_API_KEY="$(read_repo_env_value NVIDIA_API_KEY FREEWILLER_NVIDIA_API_KEY NGC_API_KEY NVIDIA_KIMI_K25_API_KEY || true)"
REPO_NVIDIA_API_BASE_URL="$(read_repo_env_value FREEWILLER_NVIDIA_API_BASE_URL NVIDIA_API_BASE_URL || echo https://integrate.api.nvidia.com/v1)"
REPO_NVIDIA_MODEL="$(read_repo_env_value FREEWILLER_NVIDIA_MODEL NVIDIA_MODEL || echo moonshotai/kimi-k2.5)"
REPO_NVIDIA_API_MODE="$(read_repo_env_value FREEWILLER_NVIDIA_API_MODE NVIDIA_API_MODE || echo chat)"
REPO_NVIDIA_EXTRA_BODY_JSON="$(read_repo_env_value FREEWILLER_NVIDIA_EXTRA_BODY_JSON NVIDIA_EXTRA_BODY_JSON || true)"
REPO_NVIDIA_MAX_OUTPUT_TOKENS="$(read_repo_env_value FREEWILLER_NVIDIA_MAX_OUTPUT_TOKENS NVIDIA_MAX_OUTPUT_TOKENS || echo 1024)"
REPO_OPENROUTER_API_KEY="$(read_repo_env_value FREEWILLER_OPENROUTER_API_KEY OPENROUTER_API_KEY || true)"
REPO_OPENROUTER_API_BASE_URL="$(read_repo_env_value FREEWILLER_OPENROUTER_API_BASE_URL OPENROUTER_API_BASE_URL || echo https://openrouter.ai/api/v1)"
REPO_OPENROUTER_MODEL="$(read_repo_env_value FREEWILLER_OPENROUTER_MODEL OPENROUTER_MODEL || echo openrouter/auto)"
REPO_OPENROUTER_API_MODE="$(read_repo_env_value FREEWILLER_OPENROUTER_API_MODE OPENROUTER_API_MODE || echo chat)"
REPO_OPENROUTER_MAX_OUTPUT_TOKENS="$(read_repo_env_value FREEWILLER_OPENROUTER_MAX_OUTPUT_TOKENS OPENROUTER_MAX_OUTPUT_TOKENS || echo 1024)"
...
FREEWILLER_USAGE_LEDGER_FILE="${FREEWILLER_USAGE_LEDGER_FILE:-$(read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_USAGE_LEDGER_FILE || echo "${TELEMETRY_DIR}/provider-usage.jsonl")}"
FREEWILLER_CHEAP_PROVIDER_FAMILY="${FREEWILLER_CHEAP_PROVIDER_FAMILY:-$(read_repo_env_value FREEWILLER_CHEAP_PROVIDER_FAMILY || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_PROVIDER_FAMILY || true)}"
FREEWILLER_CHEAP_API_BASE_URL="${FREEWILLER_CHEAP_API_BASE_URL:-$(read_repo_env_value FREEWILLER_CHEAP_API_BASE_URL || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_API_BASE_URL || true)}"
FREEWILLER_CHEAP_API_KEY="${FREEWILLER_CHEAP_API_KEY:-$(read_repo_env_value FREEWILLER_CHEAP_API_KEY || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_API_KEY || true)}"
FREEWILLER_CHEAP_MODEL="${FREEWILLER_CHEAP_MODEL:-$(read_repo_env_value FREEWILLER_CHEAP_MODEL || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_MODEL || true)}"
FREEWILLER_CHEAP_API_MODE="${FREEWILLER_CHEAP_API_MODE:-$(read_repo_env_value FREEWILLER_CHEAP_API_MODE || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_API_MODE || echo chat)}"
FREEWILLER_CHEAP_EXTRA_BODY_JSON="${FREEWILLER_CHEAP_EXTRA_BODY_JSON:-$(read_repo_env_value FREEWILLER_CHEAP_EXTRA_BODY_JSON || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_EXTRA_BODY_JSON || true)}"
FREEWILLER_CHEAP_MAX_OUTPUT_TOKENS="${FREEWILLER_CHEAP_MAX_OUTPUT_TOKENS:-$(read_repo_env_value FREEWILLER_CHEAP_MAX_OUTPUT_TOKENS || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_CHEAP_MAX_OUTPUT_TOKENS || echo 1024)}"
...
FREEWILLER_PUBLIC_PROVIDER_FAMILY="${FREEWILLER_PUBLIC_PROVIDER_FAMILY:-$(read_repo_env_value FREEWILLER_PUBLIC_PROVIDER_FAMILY || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_PROVIDER_FAMILY || true)}"
FREEWILLER_PUBLIC_API_BASE_URL="${FREEWILLER_PUBLIC_API_BASE_URL:-$(read_repo_env_value FREEWILLER_PUBLIC_API_BASE_URL || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_API_BASE_URL || true)}"
FREEWILLER_PUBLIC_API_KEY="${FREEWILLER_PUBLIC_API_KEY:-$(read_repo_env_value FREEWILLER_PUBLIC_API_KEY || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_API_KEY || true)}"
FREEWILLER_PUBLIC_MODEL="${FREEWILLER_PUBLIC_MODEL:-$(read_repo_env_value FREEWILLER_PUBLIC_MODEL || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_MODEL || true)}"
FREEWILLER_PUBLIC_API_MODE="${FREEWILLER_PUBLIC_API_MODE:-$(read_repo_env_value FREEWILLER_PUBLIC_API_MODE || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_API_MODE || echo chat)}"
FREEWILLER_PUBLIC_EXTRA_BODY_JSON="${FREEWILLER_PUBLIC_EXTRA_BODY_JSON:-$(read_repo_env_value FREEWILLER_PUBLIC_EXTRA_BODY_JSON || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_EXTRA_BODY_JSON || true)}"
FREEWILLER_PUBLIC_MAX_OUTPUT_TOKENS="${FREEWILLER_PUBLIC_MAX_OUTPUT_TOKENS:-$(read_repo_env_value FREEWILLER_PUBLIC_MAX_OUTPUT_TOKENS || read_persisted_value "$PROVIDER_ROUTING_ENV_FILE" FREEWILLER_PUBLIC_MAX_OUTPUT_TOKENS || echo 1024)}"
...
persist_config() {
  run_as_root touch "$ACCESS_ENV_FILE" "$CONF_ENV_FILE"
  run_as_root mkdir -p "$LOCAL_LLM_DIR" "$POLICY_DIR" "$TELEMETRY_DIR"
...
EOF
  ...
  run_as_root chmod 755 "$LOCAL_LLM_DIR" "$POLICY_DIR" "$TELEMETRY_DIR"
